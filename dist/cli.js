...
var SITE_HIDDEN_STATUSES, ARCHIVED_FILTER;
var init_showVisibility = __esm({
  "src/lib/showVisibility.ts"() {
    "use strict";
    SITE_HIDDEN_STATUSES = ["archived"];
    ARCHIVED_FILTER = `(${SITE_HIDDEN_STATUSES.join(",")})`;
  }
});
...
async function listPlaylistItems(accessToken, playlistId) {
  ...
  for (const row of rows ?? []) {
    ...
    if (show) {
      ...
      const artists = (show.show_artists ?? []).map((link2) => link2.artists?.name).filter((name) => !!name).join(", ");
      entries.push({
        kind: "show",
        id: show.id,
...
async function listLikedShows(accessToken, userId, limit = 50) {
  ...
  for (const row of rows ?? []) {
    ...
    shows.push({
      ...
      artists: (show.show_artists ?? []).map((link2) => link2.artists?.name).filter((name) => !!name)
    });
  }
  return shows;
}
...
var LOCK_STALE_MS, LOCK_POLL_MS;
...
var require_server = __commonJS({
  "node_modules/qrcode/lib/server.js"(exports) {
    ...
    exports.toString = function toString(text, opts, cb) {
      ...
      const renderer2 = getStringRendererFromType(type);
      return render2(renderer2.render, text, params);
    };
    exports.toDataURL = function toDataURL(text, opts, cb) {
      const params = checkParams(text, opts, cb);
      const renderer2 = getRendererFromType(params.opts.type);
      return render2(renderer2.renderToDataURL, text, params);
    };
    ...
    exports.toFile = function toFile(path, text, opts, cb) {
      ...
      const renderToFile = renderer2.renderToFile.bind(null, path);
    ...
    exports.toFileStream = function toFileStream(stream, text, opts) {
      ...
      const renderer2 = getRendererFromType("png");
      const renderToFileStream = renderer2.renderToFileStream.bind(null, stream);
...
// src/api/shows.ts
function toShow(row) {
  const artists = (row.show_artists ?? []).map((link2) => link2.artists).filter((artist) => artist !== null);
...
async function listShows(options = {}) {
  const params = new URLSearchParams({
    select: SHOW_SELECT,
    status: "eq.published",
...
async function searchShows(query, limit = 12, accessToken = null) {
  ...
  for (const link2 of artistRows ?? []) {
    const row = link2.show;
...
var cachedListenerId, SESSION_ID;
var init_ids = __esm({
  "src/lib/ids.ts"() {
    "use strict";
    init_session();
    cachedListenerId = null;
...
var init_radio = __esm({
  "src/api/radio.ts"() {
    ...
    init_backupStream();
    init_http();
    SCHEDULE_INTERVAL_MS = NOWPLAYING_INTERVAL_MS;
  }
});
...
var init_base = __esm({
  "node_modules/ansi-escapes/base.js"() {
    ...
    isTmux = !isBrowser && (process2.env.TERM?.startsWith("screen") || process2.env.TERM?.startsWith("tmux") || process2.env.TMUX !== void 0);
...
function wrapAssembly(lib) {
  ...
  patch(lib.Node.prototype, "calculateLayout", function(original) {
    let width2 = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : NaN;
    ...
    return original.call(this, width2, height, direction);
  });
  return {
    Config: lib.Config,
    Node: lib.Node,
...
var init_measure_text = __esm({
  "node_modules/ink/build/measure-text.js"() {
    ...
    measureText = (text) => {
      ...
      cache[text] = { width: width2, height };
...
var init_dom = __esm({
  async "node_modules/ink/build/dom.js"() {
    ...
    measureTextNode = function(node, width2) {
      ...
      if (dimensions.width >= 1 && width2 > 0 && width2 < 1) {
        return dimensions;
      }
      const textWrap = node.style?.textWrap ?? "wrap";
      const wrappedText = wrap_text_default(text, width2, textWrap);
...
var applyPositionStyles, applyMarginStyles, applyPaddingStyles, applyFlexStyles, applyDimensionStyles, applyDisplayStyles, applyBorderStyles, applyGapStyles, styles2, styles_default;
...
var init_devtools_window_polyfill = __esm({
  "node_modules/ink/build/devtools-window-polyfill.js"() {
    init_ws_stub();
    customGlobal = global;
    customGlobal.WebSocket ||= WebSocketStub;
...
var import_react_reconciler, import_constants, diff, cleanupYogaNode, reconciler_default;
...
var init_ansi_styles2 = __esm({
  "node_modules/chalk/source/vendor/ansi-styles/index.js"() {
    ...
    wrapAnsi16m2 = (offset = 0) => (red2, green2, blue) => `\x1B[${38 + offset};2;${red2};${green2};${blue}m`;
    styles3 = {
      modifier: {
        reset: [0, 0],
        // 21 isn't widely supported and 22 does the same thing
  ...
  "node_modules/ink/build/render-border.js"() {
    ...
    renderBorder = (x, y, node, output) => {
      if (node.style.borderStyle) {
        const width2 = node.yogaNode.getComputedWidth();
        ...
        const contentWidth = width2 - (showLeftBorder ? 1 : 0) - (showRightBorder ? 1 : 0);
        ...
        if (showRightBorder) {
          output.write(x + width2 - 1, y + offsetY, rightBorder, {
            transformers: []
          });
        }
        if (bottomBorder) {
...
var applyPaddingToText, renderNodeToOutput, render_node_to_output_default;
...
function tokenize(string, endCharacter = Number.POSITIVE_INFINITY) {
  ...
  while (index < string.length) {
    ...
    const isFullWidth = isFullwidthCodePoint2(codePoint);
...
var ESCAPES3, CODE_POINT_0, CODE_POINT_9, MAX_ANSI_SEQUENCE_LENGTH, endCodesSet, endCodesMap;
var init_slice_ansi2 = __esm({
  "node_modules/slice-ansi/index.js"() {
    init_ansi_styles();
    init_is_fullwidth_code_point2();
    ESCAPES3 = /* @__PURE__ */ new Set([27, 155]);
...
var ESCAPES4, endCodesSet2, endCodesMap2, linkStartCodePrefix, linkStartCodePrefixCharCodes, linkCodeSuffix, linkCodeSuffixCharCode, linkEndCode;
...
var create, logUpdate, log_update_default;
...
var init_AppContext = __esm({
  "node_modules/ink/build/components/AppContext.js"() {
    ...
    AppContext = (0, import_react.createContext)({
      exit() {
      }
    });
...
var init_StdinContext = __esm({
  "node_modules/ink/build/components/StdinContext.js"() {
    ...
    StdinContext = (0, import_react2.createContext)({
...
var init_StdoutContext = __esm({
  "node_modules/ink/build/components/StdoutContext.js"() {
    ...
    StdoutContext = (0, import_react3.createContext)({
...
var init_StderrContext = __esm({
  "node_modules/ink/build/components/StderrContext.js"() {
    ...
    StderrContext = (0, import_react4.createContext)({
...
var init_FocusContext = __esm({
  "node_modules/ink/build/components/FocusContext.js"() {
    ...
    FocusContext = (0, import_react5.createContext)({
      activeId: void 0,
      add() {
      },
      remove() {
...
var init_Box = __esm({
  "node_modules/ink/build/components/Box.js"() {
    ...
    Box = (0, import_react6.forwardRef)(({ children, ...style }, ref) => {
...
var import_react8, import_stack_utils, cleanupPath, stackUtils;
...
var import_react9, tab, shiftTab, escape, App;
var init_App = __esm({
  "node_modules/ink/build/components/App.js"() {
    ...
    shiftTab = "\x1B[Z";
    escape = "\x1B";
    App = class extends import_react9.PureComponent {
...
var import_react10, import_signal_exit2, noop, Ink;
var init_ink = __esm({
...
var metaKeyCodeRe, fnKeyRe, keyName, nonAlphanumericKeys, isShiftKey, isCtrlKey, parseKeypress, parse_keypress_default;
...
var import_react15, useStdin, use_stdin_default;
var init_use_stdin = __esm({
  "node_modules/ink/build/hooks/use-stdin.js"() {
    import_react15 = __toESM(require_react(), 1);
    init_StdinContext();
    useStdin = () => (0, import_react15.useContext)(StdinContext_default);
    use_stdin_default = useStdin;
  }
});

// node_modules/ink/build/hooks/use-input.js
var import_react16, useInput, use_input_default;
...
var import_react17, useApp, use_app_default;
var init_use_app = __esm({
  "node_modules/ink/build/hooks/use-app.js"() {
    import_react17 = __toESM(require_react(), 1);
    init_AppContext();
    useApp = () => (0, import_react17.useContext)(AppContext_default);
    use_app_default = useApp;
  }
});

// node_modules/ink/build/hooks/use-stdout.js
var import_react18, useStdout, use_stdout_default;
var init_use_stdout = __esm({
  "node_modules/ink/build/hooks/use-stdout.js"() {
    import_react18 = __toESM(require_react(), 1);
    init_StdoutContext();
    useStdout = () => (0, import_react18.useContext)(StdoutContext_default);
    use_stdout_default = useStdout;
  }
});

// node_modules/ink/build/hooks/use-stderr.js
var import_react19, useStderr, use_stderr_default;
var init_use_stderr = __esm({
  "node_modules/ink/build/hooks/use-stderr.js"() {
    import_react19 = __toESM(require_react(), 1);
    init_StderrContext();
    useStderr = () => (0, import_react19.useContext)(StderrContext_default);
    use_stderr_default = useStderr;
  }
});

// node_modules/ink/build/hooks/use-focus.js
var import_react20, useFocus, use_focus_default;
var init_use_focus = __esm({
  "node_modules/ink/build/hooks/use-focus.js"() {
    import_react20 = __toESM(require_react(), 1);
    init_FocusContext();
    init_use_stdin();
    useFocus = ({ isActive = true, autoFocus = false, id: customId } = {}) => {
      const { isRawModeSupported, setRawMode } = use_stdin_default();
      const { activeId, add, remove, activate, deactivate, focus } = (0, import_react20.useContext)(FocusContext_default);
      const id = (0, import_react20.useMemo)(() => {
...
var import_react21, useFocusManager, use_focus_manager_default;
var init_use_focus_manager = __esm({
  "node_modules/ink/build/hooks/use-focus-manager.js"() {
    import_react21 = __toESM(require_react(), 1);
    init_FocusContext();
    useFocusManager = () => {
      const focusContext = (0, import_react21.useContext)(FocusContext_default);
      return {
...
var measureElement, measure_element_default;
          ...
          visible.map((row, offset) => {
            ...
            return /* @__PURE__ */ (0, import_jsx_runtime5.jsx)(Box_default, { children: /* @__PURE__ */ (0, import_jsx_runtime5.jsx)(
              Text,
              {
                ...
                children: columns.map((column) => padTo(column.value(row, index), widthOf(column))).join(" ")
              }
            ) }, index);
          })
        ] })
...
function litSegments(db, segments) {
...
function segmentDb(index, segments) {
  if (segments <= 1) return METER_MIN_DB;
  const ratio = index / (segments - 1);
  return METER_MIN_DB + ratio * (METER_MAX_DB - METER_MIN_DB);
}
function decayPeak(previous, current2, fallDbPerTick) {
  if (!Number.isFinite(previous)) return current2;
  if (current2 >= previous) return current2;
  return Math.max(current2, previous - Math.abs(fallDbPerTick));
}
function scaleRow(segments, marks = [-30, -20, -10, -5, 0]) {
...
`);
  ...
  const playback = startPlayback({
    ...
    render: (state, position, total, width2) => {
      ...
      const bar2 = progressBar(shownPosition, shownTotal, Math.max(0, Math.min(24, width2 - 48)));
      const head = truncate(trackLabel(track), Math.max(10, width2 - clock.length - bar2.length - 8));
...
async function resolveShow(input, accessToken) {
  const link2 = parseSurpriseLink(input);
  if (link2) {
    if (link2.kind !== "show") {
      process.stderr.write(`${yellow("!")} \u041F\u043E\u043A\u0430 \u0443\u043C\u0435\u0435\u043C \u0438\u0433\u0440\u0430\u0442\u044C \u0442\u043E\u043B\u044C\u043A\u043E \u0432\u044B\u043F\u0443\u0441\u043A\u0438. \u0421\u0441\u044B\u043B\u043A\u0430 \u0432\u0435\u0434\u0451\u0442 \u043D\u0430 \u0434\u0440\u0443\u0433\u043E\u0435 (${link2.kind}).
`);
      return null;
    }
    return findShow(link2.param, accessToken);
  }
...
`);
  ...
  const link2 = parseSurpriseLink(query);
  if (link2?.kind === "track") {
    const trackId = link2.param.slug ?? "";
...
`);
  ...
  if (link2?.kind === "release") {
    const releaseId = link2.param.slug ?? "";
...
`);
  ...
  const playback = startPlayback({
    ...
    render: (state, position, total, width2) => {
      ...
      const bar2 = progressBar(position, duration, Math.max(0, Math.min(24, width2 - 48)));
      const head = truncate(showTitle(show), Math.max(10, width2 - clock.length - bar2.length - 8));
      return `${stateMark(state)} ${bold(head)} ${bar2 ? `${dim(bar2)} ` : ""}${dim(clock)}`;
    }
  });
  try {
...
`);
  ...
  if (degraded) {
    process.stderr.write(
      `${yellow("!")} \u0418\u0433\u0440\u0430\u0435\u043C \u0447\u0435\u0440\u0435\u0437 ${name}: \u0431\u0435\u0437 \u043F\u043B\u0430\u0432\u043D\u043E\u0439 \u043F\u0435\u0440\u0435\u043C\u043E\u0442\u043A\u0438 \u0438 \u0440\u0435\u0433\u0443\u043B\u0438\u0440\u043E\u0432\u043A\u0438 \u0433\u0440\u043E\u043C\u043A\u043E\u0441\u0442\u0438. \u041F\u043E\u0441\u0442\u0430\u0432\u044C\u0442\u0435 mpv \u2014 \u0441\u0442\u0430\u043D\u0435\u0442 \u043B\u0443\u0447\u0448\u0435.
`
    );
  }
  const session = await getValidSession();
