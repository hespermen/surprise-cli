 ...
 * Символы полосы.
 *
 * КАЖДЫЙ обязан занимать ровно одну колонку — это проверяет тест, и проверка не
 * формальность. Прежнее погашенное деление «▪» по стандарту Unicode считается
 * символом ДВОЙНОЙ ширины: 64 деления занимали 128 колонок вместо 64, строка
 * измерителя переносилась на вторую, и вся раскладка под ней съезжала.
 *
 * Хуже того, ширина строки менялась вместе с музыкой: чем громче, тем больше
 * узких «█» вместо широких «▪». Экран дрожал в такт звуку, и выглядело это как
 * необъяснимый скачущий интерфейс.
 */
