...
/**
 * Хинт платформы для telegram-login-start. Обязан входить в список bot-ветки
 * pickLoginMethod (supabase/functions/_shared/telegramVerify.ts), иначе сервер
 * ответит method:"oauth" — без poll_secret, и забрать сессию будет нечем.
 *
 * Переопределяется переменной окружения — это нужно ровно в одном случае: пока
 * правка с "cli" не выкачена на прод, вход можно проверить, подставив уже
 * известное серверу значение:
 *
 *   SURPRISE_PLATFORM_HINT=extension surprise login
 *
 * Постоянным решением это быть не должно: сервер тогда считает терминал
 * расширением, и различить их в статистике входов станет нечем.
 */
